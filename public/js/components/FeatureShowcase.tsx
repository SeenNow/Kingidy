...
const FeatureShowcase: React.FC<{ features?: Feature[]; className?: string }> = ({ features = defaultFeatures, className = '' }) => {
  const [currentFeature, setCurrentFeature] = useState(0);

  const currentFeatureData = features[currentFeature];

  const handlePrevious = () => setCurrentFeature((prev) => (prev === 0 ? features.length - 1 : prev - 1));
  const handleNext = () => setCurrentFeature((prev) => (prev === features.length - 1 ? 0 : prev + 1));
  ...
  return (
    <div className={`features-showcase ${className}`}>
      ...
      <div className="features-content-section">
        ...
        <div className="carousel-controls">
          <button className="carousel-btn prev-btn" onClick={handlePrevious} aria-label="Previous feature" title="Previous (← arrow key)">
            <i className="fas fa-chevron-left"></i>
          </button>
          <div className="carousel-indicators">
            {features.map((_, idx) => (
              <button key={idx} className={`indicator-dot ${currentFeature === idx ? 'active' : ''}`} onClick={() => setCurrentFeature(idx)} aria-label={`Go to feature ${idx + 1}`} aria-current={currentFeature === idx ? 'true' : 'false'} />
            ))}
          </div>
          <button className="carousel-btn next-btn" onClick={handleNext} aria-label="Next feature" title="Next (→ arrow key)">
